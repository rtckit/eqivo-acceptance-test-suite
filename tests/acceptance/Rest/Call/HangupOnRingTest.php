 ...
 *
 */
class HangupOnRingTest extends AbstractAcceptanceTest
{
    public static array $deferred = [];

    public function testOriginatorCancel(): void
    {
        $promise = self::$browser
            ->post(
                ...
                http_build_query([
                    ...
                    'AnswerUrl' => 'http://' . self::CONSUMER_SOCKET_ADDR . '/testOriginatorCancel/answer',
                    'HangupUrl' => 'http://' . self::CONSUMER_SOCKET_ADDR . '/testOriginatorCancel/hangup',
                    'HangupOnRing' => 0,
                ])
            )
            ...
            ->then(function ($args): PromiseInterface {
                ...
                self::$deferred['testOriginatorCancel/hangup'] = new Deferred;

                return self::$deferred['testOriginatorCancel/hangup']->promise();
            })
            ->then(function ($args): PromiseInterface {
                $this->assertIsArray($args);
                $this->assertEquals(Uuid::UUID_TYPE_RANDOM, Uuid::fromString($args['CallUUID'])->getVersion());
                ...
                $this->assertEquals('completed', $args['CallStatus']);
                $this->assertEquals('ORIGINATOR_CANCEL', $args['HangupCause']);

                self::$deferred['default/hangup'] = new Deferred;

                return self::$deferred['default/hangup']->promise();
            })
            ->then(function ($args): PromiseInterface {
                $this->assertIsArray($args);
                $this->assertEquals(Uuid::UUID_TYPE_RANDOM, Uuid::fromString($args['CallUUID'])->getVersion());
                ...
                $this->assertEquals('inbound', $args['Direction']);
                $this->assertEquals('completed', $args['CallStatus']);
                $this->assertEquals('NORMAL_CLEARING', $args['HangupCause']);
    ...
    public function testHangupOnRing(): void
    {
        $promise = self::$browser
            ->post(
                '/v0.1/Call/',
                ...
                http_build_query([
                    ...
                    'To' => 'Bob',
                    'Gateways' => 'sofia/gateway/a/',
                    'AnswerUrl' => 'http://' . self::CONSUMER_SOCKET_ADDR . '/testHangupOnRing/answer',
                    'HangupUrl' => 'http://' . self::CONSUMER_SOCKET_ADDR . '/testHangupOnRing/hangup',
                    'HangupOnRing' => 3,
                ])
            )
            ->then(function ($response): PromiseInterface {
            ...
            ->then(function ($args): PromiseInterface {
                ...
                self::$deferred['testHangupOnRing/answer'] = new Deferred;

                return self::$deferred['testHangupOnRing/answer']->promise();
            })
            ->then(function ($args): PromiseInterface {
                $this->assertIsArray($args);
                $this->assertEquals(Uuid::UUID_TYPE_RANDOM, Uuid::fromString($args['CallUUID'])->getVersion());
                ...
                self::$deferred['testHangupOnRing/hangup'] = new Deferred;
                self::$deferred['default/hangup'] = new Deferred;

                return all([
                    'aliceHangup' => self::$deferred['testHangupOnRing/hangup']->promise(),
            ...
            ->then(function ($args): PromiseInterface {
                ...
                $this->assertEquals('ORIGINATOR_CANCEL', $args['aliceHangup']['HangupCause']);
    ...
    public static function onRestXmlRequest(ServerRequestInterface $request)
    {
        ...
        switch ($path) {
            ...
            case '/testHangupOnRing/answer':
                self::$deferred['testHangupOnRing/answer']->resolve($args);
            ...
            case '/testOriginatorCancel/hangup':
                self::$deferred['testOriginatorCancel/hangup']->resolve($args);
                break;

            case '/testHangupOnRing/hangup':
                self::$deferred['testHangupOnRing/hangup']->resolve($args);
                break;
        }
