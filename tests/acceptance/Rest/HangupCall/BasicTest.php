...
namespace RTCKit\Eqivo\Tests\Acceptance\Rest\HangupCall;
...
class BasicTest extends AbstractAcceptanceTest
{
        ...
        $promise = self::$browser
            ->post(
                ...
                http_build_query([
                    ...
                    'AnswerUrl' => 'http://' . self::CONSUMER_SOCKET_ADDR . '/testHangupCall/answer',
                    'HangupUrl' => 'http://' . self::CONSUMER_SOCKET_ADDR . '/testHangupCall/hangup',
                ])
            )
            ...
            ->then(function ($args): PromiseInterface {
                ...
                self::$deferred['testHangupCall/answer'] = new Deferred;

                return self::$deferred['testHangupCall/answer']->promise();
            })
            ->then(function ($args): PromiseInterface {
                $this->assertIsArray($args);
                $this->assertEquals(Uuid::UUID_TYPE_RANDOM, Uuid::fromString($args['CallUUID'])->getVersion());
                ...
                self::$deferred['testHangupCall/hangup'] = new Deferred;
                self::$deferred['default/hangup'] = new Deferred;

                return all([
                    'aliceHangup' => self::$deferred['testHangupCall/hangup']->promise(),
    ...
    public function testHangupRequest(): void
    {
        $promise = self::$browser
            ->post(
                '/v0.1/Call/',
                ...
                http_build_query([
                    ...
                    'To' => 'Bob',
                    'Gateways' => 'sofia/gateway/a/',
                    'AnswerUrl' => 'http://' . self::CONSUMER_SOCKET_ADDR . '/testHangupRequest/answer',
                    'HangupUrl' => 'http://' . self::CONSUMER_SOCKET_ADDR . '/testHangupRequest/hangup',
                ])
            )
            ->then(function ($response): PromiseInterface {
                $body = json_decode((string)$response->getBody());
            ...
            ->then(function ($response): PromiseInterface {
                ...
                self::$deferred['testHangupRequest/hangup'] = new Deferred;

                return self::$deferred['testHangupRequest/hangup']->promise();
            });
    ...
    public static function onRestXmlRequest(ServerRequestInterface $request)
    {
        ...
        switch ($path) {
            ...
            case '/testHangupCall/answer':
                self::$deferred['testHangupCall/answer']->resolve($args);
            ...
            case '/testHangupCall/hangup':
                self::$deferred['testHangupCall/hangup']->resolve($args);
                break;
